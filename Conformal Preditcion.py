...
Train_class1 = genData(2,3,1,1,300)
Train_class2 = genData(0,0,2,-1,300)
...
Test_class1 = genData(2,3,1,1,180)
Test_class2 = genData(0,0,2,-1,180)
...
def get_conformity_by_vote(data,sample):
    ...
    ranks = sorted(np.unique(conformity_scores))
    #print(ranks)
    ...
    for i in range(len(sample)):
        ...
        ranks.append(cs_1)
        ranks = sorted(ranks)
        rank_1 = ranks.index(cs_1) + 1
        #print(rank_1)
        p_val_1 = (rank_1)/len(ranks)
        #print('%d / %d = %.2f ' %(rank_1,len(ranks),p_val_1))
        p_val.append(p_val_1)
        ranks.remove(cs_1)

        ranks.append(cs_neg1)
        ranks = sorted(ranks)
        rank_neg1 = ranks.index(cs_neg1) + 1
        #print(rank_1)
        p_val_neg1 = (rank_neg1)/len(ranks)
        #print('%d / %d = %.2f ' %(rank_neg1,len(ranks),p_val_neg1))
...
def get_conformity_by_vote_randomized(data,sample):
    np.seterr(divide='ignore')
    conformity_scores = []
    p_vals = []
    
    for x in data:
        label,impurity,node = model1.predict_for_sample(x,model.root)
        cs = node.vote
        conformity_scores.append(cs)
    ranks = sorted(np.unique(conformity_scores))
    
    for i in range(len(sample)):
        label,impurity,node = model1.predict_for_sample(sample[i],model.root)
        p_val = []
        if(label==1):
            vote_1 = node.vote
            vote_neg1 = 1 - node.vote
        ...
        for j in reversed(range(len(ranks))):
            if(cs_neg1>ranks[j]):
                if(cs_neg1==ranks[j+1]):
                    alpha = 1
                else:
                    alpha = 0
                p_val_neg1 = ((j+1)+alpha)/len(ranks)
                break
...
#CP
Y = []
p_val = get_conformity_by_vote(data,sample)
    
for i in range(len(p_val)):
    y = -1 if(p_val[i][0]<p_val[i][1]) else 1
    Y.append(y)
    #print('X: %d, %d P_val: %.2f, %.2f Y: %d \n' % (X_test[i][0],X_test[i][1],p_val[i][0],p_val[i][1],y))
...
#CP with randomized p-values
Y = []
p_val = get_conformity_by_vote_randomized(data,sample)
